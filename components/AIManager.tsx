    ...
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        <div>
          ...
          <p className="text-slate-500 mt-1">Intelligent forecasting and team performance tracking</p>
        </div>
        <button
          onClick={handleGenerateInsights}
          disabled={loading || !apiKey}
          className="flex items-center justify-center gap-2 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white px-6 py-3 rounded-xl font-bold shadow-md shadow-purple-200 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {loading ? (
            <>
      ...
      <div className="space-y-6">
        {insights ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
              <div className="p-5 border-b border-slate-100 flex items-center gap-3">
                ...
                <h2 className="text-lg font-bold text-slate-800">Batch Forecasts</h2>
              </div>
              <div className="p-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {insights.batchPredictions.map((prediction, idx) => (
                  <div key={idx} className="border border-slate-100 rounded-xl p-4 bg-slate-50/50 flex flex-col">
                    <div className="font-bold text-slate-800 mb-1 truncate" title={prediction.batchName}>
                      {prediction.batchName}
                    </div>
                    <div className="flex items-end justify-between mt-auto pt-4">
                      <div>
                        <div className="text-xs text-slate-500 font-medium uppercase tracking-wider mb-1">Remaining</div>
                        <div className="font-black text-2xl text-slate-700">{prediction.predictedDaysRemaining} <span className="text-sm font-medium text-slate-500">days</span></div>
                      </div>
                      <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${
                        prediction.status === 'On Track' ? 'bg-emerald-100 text-emerald-700' :
                        prediction.status === 'Delayed' ? 'bg-amber-100 text-amber-700' :
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
              <div className="p-5 border-b border-slate-100 flex items-center gap-3">
                ...
                <h2 className="text-lg font-bold text-slate-800">Strategic Recommendations</h2>
              </div>
              <div className="p-5">
                <ul className="space-y-3">
                  ...
                  {insights.recommendations.length === 0 && (
                    <li className="text-slate-500 text-sm italic">No recommendations available.</li>
                  )}
                </ul>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
              <div className="p-5 border-b border-slate-100 flex items-center gap-3">
                ...
                <h2 className="text-lg font-bold text-slate-800">Identified Bottlenecks</h2>
              </div>
              <div className="p-5">
                <ul className="space-y-3">
                  ...
                  {insights.bottlenecks.length === 0 && (
                    <li className="text-slate-500 text-sm italic">No bottlenecks identified.</li>
                  )}
                </ul>
              </div>
            </div>
          ...
          <div className="bg-slate-50 rounded-2xl border border-slate-200 border-dashed flex flex-col items-center justify-center p-8 text-center">
            ...
            <h3 className="text-lg font-bold text-slate-800 mb-2">AI Insights Ready</h3>
